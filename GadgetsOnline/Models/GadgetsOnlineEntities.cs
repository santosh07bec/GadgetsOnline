...
namespace GadgetsOnline.Models
{
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        {
            var databaseMode = Environment.GetEnvironmentVariable("dbmode");
            Console.WriteLine($"DB mode is {databaseMode}");

            var path = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            var DbPath = System.IO.Path.Join(path, "gadgetsonline.db");

            if (!String.IsNullOrEmpty(databaseMode))
            {
                //optionsBuilder.UseInMemoryDatabase(databaseName: "GadgetsOnlineEntities");
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Product>().HasData(
                new Product { ProductId = 1, CategoryId = 1, Name = "Phone 18", Price = 699.00M, ProductArtUrl = "/Content/Images/Mobile/1.jpg" },
